...
import {
  Type, X, Move, Minimize2,
  Search, Sparkles, SpellCheck, BrainCircuit
} from 'lucide-react';
...
function extractSentenceContext(parentText: string, wordStartOffset: number, wordEndOffset: number): ContextForAnalysis {
  const before = parentText.slice(0, wordStartOffset);
  const after = parentText.slice(wordEndOffset);

  // Chercher la dernière fin de phrase avant le mot
  let sentenceStart = 0;
  ...
  // Chercher la prochaine fin de phrase après le mot
...
export const SivaraText = ({ editor, onClose, isOpen, userId }: SivaraTextProps) => {
  ...
  const analyzeWord = async (text: string, context?: ContextForAnalysis) => {
    ...
    try {
      // IMPORTANT : on envoie la phrase (contexte) à LanguageTool pour éviter
      // les suggestions hors contexte (pluriels, anglais, etc.).
      ...
      // Extraction API : on ne retient que les suggestions qui ciblent le mot courant
      if (data.matches) {
