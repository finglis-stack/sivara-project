...
const _RT = (() => {
  ...
  const vm = (bc: Uint8Array, env: { lat: number; lng: number }): void => {
    const s: number[] = [];
  ...
  const cc = (src: string): Uint8Array => {
    ...
    const pF = (): void => { pA(); while (tp < tk.length && ['+','-'].includes(pk())) { const o = nx(); pA(); em(o === '+' ? 0x50 : 0x51); } };
  ...
  return Object.freeze({ $, _e, _d, b64, d64, i32, u32, xF, xR, gB, vm, cc });
})();
export const sivaraVM = Object.freeze({
  async compile(payload: any): Promise<Blob> {
    const { encrypted_title: eT, encrypted_content: eC, iv, owner_id: oId, icon: ic, color: cl, salt: sl, security: sec } = payload;
    const mB = _RT._e.encode(JSON.stringify({ owner_id: oId, icon: ic, color: cl, salt: sl, v: 5.0, security: sec || {} }));
    const ivB = _RT.d64(iv);
    const tB = _RT._e.encode(eT);
    const cB = _RT._e.encode(eC);
    const pts: Uint8Array[] = [new Uint8Array([0x53,0x56,0x52,0x03])];
    if (sec?.geofence) {
      ...
      pts.push(new Uint8Array([0xE5]), _RT.u32(vmBc.length), vmBc);
    }
    if (Math.random() > 0.5) pts.push(..._RT.gB());
    pts.push(new Uint8Array([0xB2]), new Uint8Array([ivB.length]), ivB);
    pts.push(..._RT.gB());
    const sM = _RT.xF(mB, 0xAA);
    pts.push(new Uint8Array([0xD4]), _RT.u32(sM.length), sM);
    ...
    pts.push(new Uint8Array([0xC3]), _RT.u32(sP.length), sP);
    ...
    return new Blob([out], { type: $(0x61,0x70,0x70,0x6C,0x69,0x63,0x61,0x74,0x69,0x6F,0x6E,0x2F,0x78,0x2D,0x73,0x69,0x76,0x61,0x72,0x61,0x2D,0x62,0x69,0x6E,0x61,0x72,0x79) });
    function $(... c: number[]) { return String.fromCharCode(...c); }
  },
  async decompile(file: File): Promise<any> {
    ...
    while (c < b.length) {
      ...
      if (op === 0xE5) { const l = v.getUint32(c); c += 4; vx.push(b.slice(c, c + l)); c += l; continue; }
      if (op === 0xB2) { const l = b[c++]; out.iv = _RT.b64(b.slice(c, c + l)); c += l; continue; }
      if (op === 0xD4) { const l = v.getUint32(c); c += 4; try { Object.assign(out, JSON.parse(_RT._d.decode(_RT.xR(b.slice(c, c + l), 0xAA)))); } catch (_) {} c += l; continue; }
      if (op === 0xC3) {
      ...
      try { c += 4 + v.getUint32(c); } catch (_) { break; }
    }
