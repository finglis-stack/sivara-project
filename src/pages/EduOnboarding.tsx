  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      confetti({
        ...
        spread: 100,
        origin: { y: 0.6 },
        colors: ['#3B82F6', '#ffffff', '#FCD34D'] // Bleu, Blanc, Jaune (Espace)
      });
    ...
    <div className={`min-h-screen flex flex-col items-center justify-center p-6 font-sans transition-all duration-1000 ease-in-out relative overflow-hidden ${isSpaceTheme ? 'text-white' : 'bg-white text-black'}`}>
      ...
      {isSpaceTheme && (
          <div className="absolute inset-0 z-0">
              <div 
                  className="absolute inset-0 bg-cover bg-center animate-in fade-in duration-1000 scale-105"
                  style={{ backgroundImage: 'url(/space-shuttle.jpg)' }}
              />
              {/* Overlay sombre + flou */}
              <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] transition-all duration-1000"></div>
          </div>
      )}

      {/* Logo Minimaliste Fixe */}
      <div className="absolute top-8 left-8 flex items-center gap-3 opacity-0 animate-in fade-in duration-1000 z-20">
        ...
        {step === 0 && (
            <div className="text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
                <h1 className="text-5xl md:text-7xl font-bold mb-8 tracking-tighter leading-[0.9]">
                    Hacker <br/>
                    <span className={isSpaceTheme ? 'text-blue-300' : 'text-gray-400'}>vos études.</span>
                </h1>
