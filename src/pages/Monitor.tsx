...
const Monitor = () => {
  ...
  // --- WATCHDOG (AUTO-KICK) ---
  useEffect(() => {
    // Ce chien de garde vérifie toutes les 5 secondes si la file est bloquée
    const watchdog = setInterval(() => {
      if (!isActive) return; // Ne rien faire si pause

      const pending = queue.filter(i => i.status === 'pending').length;
      const processing = queue.filter(i => i.status === 'processing').length;

      // S'il y a du travail en attente MAIS personne ne travaille (et qu'on n'est pas en pause)
      if (pending > 0 && processing === 0) {
        console.log('🐶 Watchdog: Queue stuck? Kicking process-queue...');
        triggerProcessQueue();
      }
    }, 5000);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 flex items-center gap-3 mt-1.5">
                ...
                {pendingCount > 0 && processingCount === 0 && isActive && (
                   <span className="flex items-center gap-1 text-xs text-orange-500 font-bold animate-pulse"><Zap className="h-3 w-3" /> Redémarrage auto...</span>
                )}
              </p>
            </div>
          </div>
